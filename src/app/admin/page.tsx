...
export default function AdminPage() {
  ...
  const handleResetRehearsal = async () => {
    if (!session) return;
    if (!confirm('Reset rehearsal session? This will wipe all test bids, restore all team purses to ₹100k, and go back to Lot #1.')) return;

    setIsProcessing(true);
    try {
      const res = await resetRehearsalSessionAction(session.id);
      if (res.success) {
        setOpMessage('Rehearsal data reset. Returned to Lot #1 with all purses restored to ₹100k.');
