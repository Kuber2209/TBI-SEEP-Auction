  ...
  return (
    <div className="rounded-xl p-6 sm:p-8 bg-[#eff4f0] border border-[#cad7cc] shadow-sm flex flex-col justify-between h-full transition-colors duration-150">
      <div className="space-y-6">
        ...
        <div>
          <h2 className="text-2xl sm:text-3xl font-semibold text-[#203126] tracking-tight text-balance">
            {startup.name}
          </h2>
          {startup.tagline && (
            <p className="text-sm sm:text-base text-[#56695e] mt-1.5 leading-snug">
              {startup.tagline}
            </p>
          )}
        </div>
        ...
        {/* Description */}
        {startup.description && (
          <p className="text-xs sm:text-sm text-[#56695e] leading-relaxed text-pretty">
            {startup.description}
          </p>
        )}
      </div>
      ...
      <div className="mt-8 pt-5 border-t border-[#cad7cc] flex items-baseline justify-between">
        <div>
          <span className="text-xs text-[#56695e] block font-medium">
            Base Floor Price
          </span>
          <span className="text-[11px] text-[#56695e]">
            Minimum reserve
          </span>
        </div>
        <div className="text-right">
          <span className="text-base sm:text-lg font-semibold text-[#203126] font-mono tabular-nums">
